...
# 版本历史
VERSION_HISTORY = {
    "4.0.2": "稳定版本：翻译与分析模型完全由环境变量或配置文件提供，不在代码和工作流中硬编码模型名称",
    "4.0.1": "稳定版本：修复数据完整性与监控语义，隔离 GitHub Pages 导航和外发内容，完善企业微信交付边界",
    "4.0.0-rc.1": "候选版本：超时与并发隔离、标题/摘要翻译保留并关闭全文翻译、财经页面修复；全功能白盒测试与线上验收仍在候选门禁中",
    "3.1.3": "修复版本：推送历史报告页脚「最后更新」补齐北京时间标注（原为裸 UTC 时间，与文件时间约定不一致）",
    "3.1.2": "修复版本：推送延迟统计错误（预期时间与 cron 不符）、跨 UTC 午夜负延迟，推送历史报告补齐北京时间标注",
